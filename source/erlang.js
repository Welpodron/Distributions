...
var x = d3.scaleLinear().domain([-2, 8]);
...
function update(elements, intervals, lambda, k) {
  let data1 = Array(parseInt(elements))
    .fill()
    .map((_) => erlang(lambda, k));
  let data2 = Array(parseInt(elements))
    .fill()
    .map((_) => erlang(lambda, k + 2));
  let data3 = Array(parseInt(elements))
    .fill()
    .map((_) => erlang(lambda, k + 4));
  ...
  x.domain([-2, Math.ceil(Math.max.apply(null, data3)) + 2]).range([0, width]);
